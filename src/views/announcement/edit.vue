<template>
  <v-container fluid class="announcements_page px-6 pl-md-14 pt-9">
    <v-row class="page_title">
      ...
      <div v-if="announcement" class="col-12 col-md-5">
        <div class="row">
          <div class="col-12">
            <div class="announcement-container">
              ...
              <div class="details">
                ...
                <div class="targert">{{ selected_target_id }}</div>
          ...
          <div class="col-12">
            ...
            <!--            <div class="input-group">-->
            <!--              <SelectUi-->
            <!--                  v-if="selected_target_id !== ''"-->
            <!--                  :label="selected_target_id"-->
            <!--                  name="role"-->
            <!--                  id="target_id"-->
      ...
      <div v-if="announcement" class="col-12 col-md-7 pt-0">
  ...
  methods: {
    ...
    async computeOptions() {
      ...
      if (this.selected_target_type === 'college') {
        this.select_options = [this.$store.state.sidebar_navbar.college.name]
        this.selected_target_id = this.$store.state.sidebar_navbar.college.name
      } else if (this.selected_target_type === 'student group') {
        if (this.$store.state.user.user.category.name === 'INSTRUCTOR') {
          const res = await Apis.get('user_groups/user')
          this.student_groups = res.data.data
        } else {
          const res = await Apis.get('user_groups/college/ALL')
          this.student_groups = res.data.data
        }

        this.student_groups.map(x => {
          this.select_options.push(x.name)
          if (x._id.toString() === this.announcement.target.id)
            this.selected_target_id = x.name
        })
        ...
        if (this.$store.state.user.user.category.name === 'INSTRUCTOR') {
          ...
          const res = await Apis.get('course/college')
          this.courses = res.data.data
        }
    ...
    async saveAnnouncement() {
      ...
      const response = await Apis.update('announcement',this.$route.params.id, {
  ...
  watch: {
    ...
    announcement() {
      if (this.announcement)
        this.selected_target_type = this.announcement.target.type === 'student_group' ? 'student group' : this.announcement.target.type
    }
  },
  async created() {
