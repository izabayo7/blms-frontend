                ...
                <v-tabs background-color="white" color="#ffd248" right>
                    ...
                    <v-tab-item v-for="n in 3" :key="n">
                        ...
                        "
                                            :color="primary"
                                            class="white--text"
                                            @click="markAsCompleted">Mark as complete</v-btn>
                                </v-col>
                                <v-col class="col-12">
                                    <v-row>
                                        <v-col class="col-6">
                                            <v-btn
                                                    v-if="activeIndex > 0"
                                                    rounded
                                                    @click="changeActiveChapter({index:activeIndex - 1,id:prevChapter(activeIndex)})"
                                                    elevation="0"
                            ...
                            "
                                                    :color="primary"
                                                    class="white--text next-chapter"
                                                    @click="changeActiveChapter({index:activeIndex + 1,id:nextChapter(activeIndex)})"
                                                    rounded
                                            >Next chapter</v-btn
                                            >
                                        </v-col>
    ...
    export default {
        ...
        methods:{
            ...
            nextChapter(idx){
                ...
                return (index < this.chapter.length-1) ? this.chapters[index+1] :
                    (index === this.chapter.length) ? this.chapters[index] : null

            },
            prevChapter(idx){
                const index = this.chapters.indexOf(idx)
                return (index > 0) ? this.chapters[index+1] :
                    (index === 0) ? 0 : null
            },
            markAsCompleted(){
                this.finish_chapter(this.$store.state.user.user.user_name).then((d) => {
                        this.maximumIndex = Math.round((d.progress * this.course.chapters.length) / 100);
                        this.changeActiveChapter({index:this.activeIndex + 1, id:this.nextChapter(this.activeIndex)})
                    }
                )
