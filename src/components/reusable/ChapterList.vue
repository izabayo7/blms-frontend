    ...
    <v-row class="fill-height">
      <v-col class="chapters px-8">
        ...
        >
          ...
          back {{ userCategory === 'ADMIN' ? '' : 'to courses' }}
        </v-btn>
        ...
        <v-list subheader class="chapters-list ml-n3">
          <v-list-item
              ...
              :key="i"
              :disabled="limit && userCategory === 'STUDENT' ? chapters[i]._id > limit : i >  maximumIndex"
...
export default {
  ...
  computed: {
    ...
    limit() {
      const chapter = this.chapters.filter(x => x.status === 0)
