<template>
    <div class="container npadding">
        <div class="c-video">
            <video @loadstart="loading()" @click="tooglePlayPause()"  @timeupdate="updateTime()" :class="`video ${showControls}`" @canplay="ready()" :src="src"></video>
  ...
  export default {
    ...
    methods:{
        loading () {
            ...
            video.setAttribute('poster', 'http://127.0.0.1:5500/Kurious-Frontend/src/assets/images/comp.jpg')
        },
        ready () {
            const video = document.querySelector('.video')
            video.removeClass('background')
