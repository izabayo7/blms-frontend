  ...
  >
    <v-list-item>
      <v-list-item-icon v-if="$store.state.sidebar.minivariant" class="hidden-sm-and-down">
        <v-app-bar-nav-icon
          @click="changeStyles('expand')"
          @click.stop="$store.state.sidebar.minivariant = !$store.state.sidebar.minivariant"
        ></v-app-bar-nav-icon>
      ...
      <v-list-item-content v-else>
        <v-list-item-title class="title">
          <v-app-bar-nav-icon
            @click="changeStyles('reduce')"
            @click.stop="$store.state.sidebar.minivariant = !$store.state.sidebar.minivariant"
            class="hidden-sm-and-down"
          ></v-app-bar-nav-icon>
          <v-list-item-title class="title hidden-md-and-up">Kurious</v-list-item-title>
  ...
  computed: {
    ...
    items() {
      if (this.$store.state.user.category === "Student") {
        ...
        return [
          { title: "Admin", icon: "mdi-account-multiple", to: "/admin" },
          { title: "Courses", icon: "mdi-projector-screen", to: "/courses" },
      ...
      if (this.breakPoint === "lg" && this.sideBar === true) {
        setTimeout(() => {
          document.getElementById("mainContent").style =
            "padding: 48px 0px 0px 15% !important;";
          document.getElementById("navbar").style =
            "height: 48px; margin-top: 0px; transform: translateY(0px); left:  15%; right: 0px !important;";
        }, 100);
      }
      if (this.breakPoint === "md" && this.sideBar === true) {
        setTimeout(() => {
          document.getElementById("mainContent").style =
            "padding: 48px 0px 0px 154px !important;";
          document.getElementById("navbar").style =
            "height: 48px; margin-top: 0px; transform: translateY(0px); left:  154px; right: 0px !important;";
        }, 100);
      }
    },
  },
    ...
    changeStyles(state) {
      if (state === "expand") {
        document.getElementById(
          "mainContent"
        ).style = `padding: 48px 0px 0px 15%;`;
        document.getElementById(
          "navbar"
        ).style = `height: 48px; margin-top: 0px; transform: translateY(0px); left: 15%; right: 0px;`;
      } else {
        document.getElementById("mainContent").style =
          "padding: 48px 0px 0px 78px;";
        document.getElementById("navbar").style =
          "height: 48px; margin-top: 0px; transform: translateY(0px); left: 78px; right: 0px;";
      }
    },
  },
};
